...
export class Polygon {
    ...
    rays = $derived.by(() => {
        ...
        for (let i = 0; i < this.edges.length; i++) {
            ...
            ray2.edgeIndex = i;
            let shortestIntersection1 = null;
            let shortestDistance1 = Infinity;
            let shortestIntersection2 = null;
            ...
            for (let j = 0; j < this.edges.length; j++) {
                ...
                if (intersection1) {
                    const dist = Math.sqrt(Math.pow(intersection1.x - midpoint.x, 2) +
                ...
                if (intersection2) {
                    const dist = Math.sqrt(Math.pow(intersection2.x - midpoint.x, 2) +
    ...
    motif = $derived.by(() => {
        ...
        for (let i = 0; i < this.rays.length; i++) {
            ...
            if (isOpposite) {
                ...
                const originsDistance = Math.sqrt(originsDistanceSquared);
                allPairs.push({
                    ray1,
                    ray2,
            ...
            if (intersection) {
                ...
                const distPDSquared = Math.pow(intersection.x - ray2.origin.x, 2) +
    ...
    motifPolygons = $derived.by(() => {
        ...
        const pointsEqual = (p1, p2) => Math.abs(p1.x - p2.x) < EPSILON && Math.abs(p1.y - p2.y) < EPSILON;
