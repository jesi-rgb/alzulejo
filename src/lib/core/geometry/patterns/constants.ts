...
export function isApproximately(value: number, target: number, tolerance = 1e-10): boolean {
  return Math.abs(value - target) < tolerance;
}

export function identifyConstant(value: number): string | null {
  ...
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.SQRT_3_OVER_2, tolerance)) return '-SQRT_3_OVER_2';
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.SQRT_3, tolerance)) return '-SQRT_3';
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.ONE_OVER_SQRT_3, tolerance)) return '-ONE_OVER_SQRT_3';
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.HALF, tolerance)) return '-HALF';
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.ONE_SIXTH, tolerance)) return '-ONE_SIXTH';
  if (isApproximately(value, -GEOMETRIC_CONSTANTS.ONE_THIRD, tolerance)) return '-ONE_THIRD';
